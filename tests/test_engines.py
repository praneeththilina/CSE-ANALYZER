...
class TestCoreEngines(unittest.TestCase):
    ...
    def test_calculate_dividend_discount_model(self):
        # Default parameters: growth=0.05, required_return=0.13
        # d1 = 10.0 * 1.05 = 10.5
        # ddm = 10.5 / (0.13 - 0.05) = 10.5 / 0.08 = 131.25
        val_default = FundamentalEngine.calculate_dividend_discount_model(10.0)
        self.assertEqual(val_default, 131.25)

        # Custom parameters
        # d1 = 5.0 * (1.0 + 0.04) = 5.20
        # ddm = 5.20 / (0.12 - 0.04) = 5.20 / 0.08 = 65.0
        ...
        # Zero dividend per share -> None
        self.assertIsNone(FundamentalEngine.calculate_dividend_discount_model(0.0))

        # Negative dividend per share -> None
        self.assertIsNone(FundamentalEngine.calculate_dividend_discount_model(-5.0))

        # Required return equal to growth rate -> None (prevents division by zero)
        self.assertIsNone(
            FundamentalEngine.calculate_dividend_discount_model(
        ...
        # Required return less than growth rate -> None
        self.assertIsNone(
            FundamentalEngine.calculate_dividend_discount_model(
                dividend_per_share=10.0,
